...
contract KswapPairBase is KswapERC20 {
    ...
    function invest(uint amount0, uint amount1) external payable lock returns (uint liquidity, uint realAmount0, uint realAmount1) {
        ...
        require(msg.value >= realAmount0, 'Kswap: AMOUNT0_APPROVE_ERROR');
    ...
    function swap(uint amount0In, uint amount1In) external payable lock returns (uint amount0Out, uint amount1Out) {
        ...
        uint _feeRate = feeRate;
        {
            ...
            if (amount0In > 0) { // baseSymbol
                ...
                require(msg.value >= amount0In, 'Kswap: AMOUNT0_APPROVE_ERROR');
            ...
            if (amount1In > 0) {
                ...
                msg.sender.transfer(amount0Out);
            }
        }
        totalFee += fee;
    ...
    function skim(address to) external onlyMinter lock {
        address _token1 = token1;
        to.transfer(address(this).balance.sub(reserve0));
